...
namespace GeekShopping.CartAPI.Repository
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<CartVO> FindCartByUserId(string userId)
        {
            ...
            cart.CartDetails = _context.CartDetails.Where(c => c.CartHeaderId == cart.CartHeader!.Id).Include(p => p.Product);
        ...
        public async Task<CartVO> SaveOrUpdateCart(CartVO cartVO)
        {
            ...
            else
            {
                var cartDetails = await _context.CartDetails.AsNoTracking()
                    .FirstOrDefaultAsync(p => p.ProductId == cartVO.CartDetails!.FirstOrDefault()!.ProductId && p.CartHeaderId == cartHeader.Id);
                if (cartDetails == null)
                {
